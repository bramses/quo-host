...
def choose_quote(quotes, transcription):
    try:
        ...
        print(e)
        return {"index": -1, "reasoning": "Something went wrong. Please try again."}
...
def bold_quote(quote, transcription, reasoning):
    try:
        ...
        print(e)
        return {"edited_quote": "Something went wrong. Please try again."}


def predict_paragraph(transcription):
    '''
    while user is speaking, predict the next paragraph of the transcription. return the next paragraph so we can preload a search
    '''
    try:
        ...
        return json.loads(completion.choices[0].message.content.replace("```json", "").replace("```", "").strip())["next_paragraph"]
    except Exception as e:
        print(e)
